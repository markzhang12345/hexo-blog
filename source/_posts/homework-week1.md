---
title: "[OurEDA]web-intro"
tag: [eda,web] 
categories: 笔记
toc: true
cover: https://th.bing.com/th/id/R.c17142d5e79a49b257bb96c767402013?rik=3pDP5rsKarZBfQ&riu=http%3a%2f%2fwww.moneyoutline.com%2fwp-content%2fuploads%2f2019%2f12%2fWeb-Design-1-1024x542.jpg&ehk=IdoQKD4jCKwkEiUMoBmmHfdxlxiCdP1ywLn2wTFO7ok%3d&risl=&pid=ImgRaw&r=0
---

# 学习Web，你可能需要的网站

## 路线图

* [Roadmap.sh](https://roadmap.sh/)

一个在 Github 中收获 234k stars 的开源文档，为各个发展方向的开发者提供了极尽翔实的学习路线推荐。
学习 Web 开发推荐优先阅读 `Frontend` 和 `Backend` 两章

* [前端路线](https://objtube.gitee.io/front-end-roadmap/#/)

一位 B 站 Up 开源的前端学习路线

## Web 历史

以史为鉴，可以明得失

* [互联网的前世今生：Web 1.0、2.0、3.0](https://www.cnblogs.com/JasonCeng/p/15861645.html)

* [JavaScript 语言的历史](https://wangdoc.com/javascript/basic/history)

## 计算机网络

计算机网络是一门在大二下学期开设的必修课程，从事 Web 开发需要有扎实的计算机网络知识功底

* [互联网是如何工作的](https://developer.mozilla.org/zh-CN/docs/Learn/Common_questions/Web_mechanics/How_does_the_Internet_work)

MDN 官方文档对互联网工作机理的解释

* [图解网络](https://xiaolincoding.com/network/#%E9%80%82%E5%90%88%E4%BB%80%E4%B9%88%E7%BE%A4%E4%BD%93)

一份国人撰写的计算机网络知识图解，内容深度适中，适合长期学习

* [HTTP 概述](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Overview)

MDN 对 HTTP 的解释

* [HTTP 的发展](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Basics_of_HTTP/Evolution_of_HTTP)

HTTP 发展历史，建议着重学习 HTTP 1.0, HTTP 1.1, HTTP 2.0

* [计算机网络](https://github.com/CyC2018/CS-Notes/blob/master/notes/%E8%AE%A1%E7%AE%97%E6%9C%BA%E7%BD%91%E7%BB%9C%20-%20%E7%9B%AE%E5%BD%95.md)

Github 上由国人开源的计算机科学知识总结，现有 stars 163k，为中文开源项目之最，在该文档中计算机网络知识和 HTTP 被分开讲解，阅读时请注意

## HTML / CSS

* [MDN 官方 HTML 教程](https://developer.mozilla.org/zh-CN/docs/Web/HTML)

* [MDN 官方 CSS 教程](https://developer.mozilla.org/zh-CN/docs/Web/CSS)

* [学习 CSS 布局](https://zh.learnlayout.com/)

一个交互式学习 CSS 布局的网站

* [HTML & CSS 编码规范](https://codeguide.bootcss.com/)

再次强调编码规范的重要性

## JavaScript

* [MDN 官方 JavaScript 教程](https://developer.mozilla.org/zh-CN/)

* [现代 JavaScript 教程](https://zh.javascript.info/)

* [ES6 入门教程(第 3 版)](https://es6.ruanyifeng.com/)

* [JavaScript 代码风格指南](https://google-styleguide.readthedocs.io/zh_CN/latest/google-javascript-styleguide/contents.html)

## Node.js

* [官方网站](https://nodejs.org/zh-cn/)

* [官方文档](https://nodejs.org/zh-cn/docs/)

* [七天学会 Node.js](http://nqdeng.github.io/7-days-nodejs/)

## Git

使用 Git 是开发者的必修课

* [Git 简明指南](http://rogerdudler.github.io/git-guide/index.zh.html)

* [Git 参考手册](http://gitref.justjavac.com/)

* [CSNote-Git](https://github.com/CyC2018/CS-Notes/blob/master/notes/Git.md)

* [图解 Git](http://marklodato.github.io/visual-git-guide/index-zh-cn.html)

* [Git 模拟器](https://oschina.gitee.io/learn-git-branching/)

一个用游戏方式学习 `Git` 的在线应用，强烈推荐

## 后记

无论致力于何种开发方向，以下技能都是必须掌握的:

* 自学 & 终身学习
* Linux 操作系统与命令行
* 熟练使用 IDE (如 Jetbrains 系 IDE，Vscode 严格来说不属于 IDE 但依然是宇宙第一)
* 英文读写
* 版本控制工具和开源仓库的使用
* 阅读 API
* 阅读文档
* 撰写 Markdown 文档
* 规范代码格式
* 数据结构与算法
* 答辩 (字面意思)
